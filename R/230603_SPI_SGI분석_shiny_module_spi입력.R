    ...
    fluidPage(
        sidebarLayout(
            sidebarPanel(
                ...
                br(),
                numericInput(NS(id, "n"), "예측할 개월수와 SPI 입력", value = 1, min = 1, max = 3),
                uiOutput(ns("spi_months")),
...
input_SPI_SGI_server <- function(id) {
    moduleServer(id, function(input, output, session) {
        ...
        # 미래 spi 갯수
        spi_future <- reactive(paste0("spi_future", seq_len(input$n)))
        
        # 미래 spi 입력 ui
        output$spi_months <- renderUI({
            ns <- session$ns
            map(spi_future(), ~ sliderInput(input = ns(.x), 
                                            label = paste0("SPI ", 
                                                           str_sub(.x, start = 11, 
                                                                   end = 11), "개월차"), 
        ...
        num_spi_future <- reactive({
            map(spi_future(), ~ input[[.x]]) %>% 
                cbind()
        })
        ...
        observeEvent(input$run_button, {
            ...
            model <- keras_model_sequential() %>%
                layer_lstm(units = 50, input_shape = c(dim(x_train2())[2], dim(x_train2())[3]),
            ...
            x_future1 <- reactive({
                array(num_spi_future(), dim = c(1, length(num_spi_future()), 1))
            })
            
            x_future2 <- reactive({
                array(x_future1() %>% as.numeric, dim = c(input$n, 1))
            })
            
            predictions <- reactive({
                model %>% predict(x_future2())
            })
            
            df_sgi_predic <- reactive({
                data.frame(sgi_predic = predictions()) %>%
                    bind_cols(data.frame(ym = df_sgi_temp()$Index + days(length(num_spi_future()) * 30)) %>%
                                  tail(length(num_spi_future())) %>% mutate_at(vars(ym), ymd)) %>%
            ...
            output$prediction_plot <- renderPlotly({
                ggplotly(
                    ggplot() +  geom_line(data = df_spi_filtered(), 
                        ...
                        geom_line(data = data.frame(SPI_future = num_spi_future() 
                                                    %>% unlist()) %>%
                                      bind_cols(ym = df_sgi_predic()$ym) %>%
                                      bind_rows(df_spi_filtered() %>%
                                                    tail(1) %>%
                                                    select(ym, spi) %>%
                                                    rename(SPI_future = spi)),
                                  aes(x = ym, y = SPI_future, color = "SPI_future")) +
                        scale_color_manual(values = c("SPI" = "steelblue",
                                                      "SGI" = "orangered",
                                                      "SGI_predic" = "orange",
                                                      "SPI_future" = "deepskyblue")) +
                        xlab("Date") +
                        ylab("SGI") +
                        theme_minimal() +
                        scale_x_date(limits = c(max(df_sgi_temp()$Index[1], 
